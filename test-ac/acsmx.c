...
{
  fprintf (stdout, "-------------match:%s-----------index:%d--\n", (char *) id,index);
  return 0;
}
...
int main (int argc, char **argv)
{
  ...
  if (argc < 3)
    ...
    {
      fprintf (stderr,
        "Usage: acsmx wordtext partern-1 partern-2 ..... partern-n -nocase\n");
      exit (0);
    }
  ...
  for (i = 2; i < argc; i++)

    {
      if (argv[i][0] == '-')
    	continue;
      printf("----------------partern:%s---------\n",argv[i]);
      acsmAddPattern (acsm, argv[i], strlen (argv[i]), nocase, 0,0,1,
            argv[i], i - 2);
    }
  int result=acsmCompile (acsm,NULL,NULL);
